module AuthenticatedTestHelper
  ...
  def authorize_as(<%= user_singular %>)
    if <%= user_singular %>
      @request.env["HTTP_AUTHORIZATION"] = "Basic #{Base64.encode64("#{<%= user_plural %>(<%= user_singular %>).login}:test")}"
...
class HttpLoginProxy < BaseLoginProxy
  protected
    ...
    def check
      @controller.assert_redirected_to :controller => '<%= session_plural %>', :action => 'new'
    end
end
